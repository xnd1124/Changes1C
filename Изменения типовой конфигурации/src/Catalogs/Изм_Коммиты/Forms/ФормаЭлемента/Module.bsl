...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) 
	МакетКонсоли = ПолучитьОбщийМакет("bsl_console");
...
Функция ВерсииФайла(Коммит, Файл)
	...
	Результат.Вставить("НоваяВерсия", РаботаСГИТ.КомандаГИТ(КомандаИзвлечения, Каталог));
	
	ШаблонИзвлеченияФайла = "show %1~1:%2";
	КомандаИзвлечения = СтрШаблон(ШаблонИзвлеченияФайла, Коммит, Файл);   
	РезультатРаботы = РаботаСГИТ.КомандаГИТ(КомандаИзвлечения, Каталог, Ложь);
