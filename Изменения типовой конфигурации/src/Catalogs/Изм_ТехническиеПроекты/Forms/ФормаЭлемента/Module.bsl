...
Процедура ПодготовитьФайлСДанными()
	...
	ДанныеДляУзла = ДанныеДляУзла(Объект.Ссылка);
	ДобавитьОписаниеУзла(СтруктураДанных, ДанныеДляУзла);
	СтрокаJSON = ЗаписатьЗначениеJSON(СтруктураДанных);
	СодержимоеФайла = "const data = " + СтрокаJSON;
...
Функция ДанныеДляУзла(Ссылка)
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	Изм_ТехническиеПроекты.Ссылка = &Ссылка";
...
&НаКлиенте
Процедура ДобавитьОписаниеУзла(СтруктураДанных, Данные)
	ОписаниеУзла = Новый Структура;
	ОписаниеУзла.Вставить("id", Строка(Данные.УникальныйИдентификатор));
	ОписаниеУзла.Вставить("type", "rect");
	ОписаниеУзла.Вставить("label", Данные.Наименование);
	ОписаниеУзла.Вставить("size", РазмерУзла(Данные));
	СтруктураДанных.Nodes.Добавить(ОписаниеУзла);
КонецПроцедуры
